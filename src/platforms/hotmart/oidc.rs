...
pub async fn exchange_with_cookies(cookies: &[(String, String)]) -> anyhow::Result<OidcUser> {
    ...
    for _ in 0..MAX_SSO_HOPS {
        ...
        let Some(location) = location else {
            ...
            return Err(anyhow!(
                "SSO authorize returned status {status} without a redirect"
            ));
        };
    ...
    let code =
        code.ok_or_else(|| anyhow!("SSO never redirected back with an authorization code"))?;
...
mod tests {
    ...
    fn redirect_code_requires_consumer_host_and_state() {
        assert_eq!(
            code_from_redirect(
                "https://consumer.hotmart.com/auth/login?code=OC-9&state=st",
                "st"
            )
            .as_deref(),
            Some("OC-9")
        );
        assert!(code_from_redirect(
            "https://consumer.hotmart.com/auth/login?code=OC-9&state=other",
            "st"
        )
        .is_none());
        assert!(
            code_from_redirect("https://evil.example/auth/login?code=OC-9&state=st", "st")
                .is_none()
        );
