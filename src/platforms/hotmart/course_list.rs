...
fn is_empty_page(body: &Value) -> bool {
    match body {
        Value::Object(map) => {
            !map.is_empty() && map.values().all(|v| !v.is_array() && !v.is_object())
        }
        _ => false,
    }
}
...
fn find_course_array<'a>(
    value: &'a Value,
    path: &str,
    depth: usize,
) -> Option<(&'a Vec<Value>, String)> {
    ...
    match value {
        Value::Array(arr) => {
            if !arr.is_empty()
                ...
                && arr.iter().any(looks_like_product)
            {
...
fn looks_like_product(entry: &Value) -> bool {
    ...
    candidates
        .iter()
        .any(|v| v.is_object() && extract_id(v).is_some() && extract_name(v).is_some())
}
...
fn parse_entry(entry: &Value) -> Option<Course> {
    let product = entry
        ...
        .filter(|p| p.is_object())
        .unwrap_or(entry);
    let id = extract_id(product)
        .or_else(|| extract_id(entry))
        .unwrap_or(0);
...
mod tests {
    ...
    fn parses_data_container() {
        ...
        assert_eq!(
            parsed.courses[0].image_url.as_deref(),
    ...
    fn parses_nested_known_containers() {
        ...
        assert_eq!(
            parse_course_list(&body).unwrap().container,
            "data.purchases"
        );
    ...
    fn slug_from_club_url_and_external_url_detection() {
        ...
        assert_eq!(
            parsed.courses[1].external_url.as_deref(),
            Some("https://members.othersite.com/course")
        );
    }
    ...
    fn real_purchase_shape_from_consumer_api() {
        ...
        assert!(
            parsed.courses[1].external_url.is_none(),
            "club.hotmart.com is not external"
        );
    ...
    fn product_details_expose_price_slug_and_memberkit_url() {
        ...
        assert_eq!(
            d.external_url.as_deref(),
            Some("https://dev-eficiente.memberkit.com.br/")
        );
    ...
    fn club_slug_from_link_forms() {
        assert_eq!(
            club_slug_from_url("https://escola.club.hotmart.com/lesson/x").as_deref(),
            Some("escola")
        );
        assert_eq!(
            club_slug_from_url("https://hotmart.com/pt-br/club/cppmasterclass/products/3941453/")
                .as_deref(),
            Some("cppmasterclass")
        );
        assert_eq!(
            club_slug_from_url("https://hotmart.com/club/abc").as_deref(),
            Some("abc")
        );
    ...
    fn skips_entries_without_id_or_name() {
        let body =
            json!({ "data": [ { "product": { "id": 0, "name": "" } }, purchase(9, "Real") ] });
        let parsed = parse_course_list(&body).unwrap();
        assert_eq!(parsed.courses.len(), 1);
    ...
    fn unrecognized_format_reports_keys_only() {
        ...
        assert_eq!(
            err.shape,
            "{profile:{email:string, plans:[2×number]}, token:string}"
        );
