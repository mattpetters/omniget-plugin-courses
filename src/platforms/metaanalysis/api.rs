...
fn base_headers() -> anyhow::Result<HeaderMap> {
    ...
    headers.insert(
        "Referer",
        HeaderValue::from_static("https://app.metaanalysisacademy.com/"),
    );
    Ok(headers)
}
...
) -> anyhow::Result<(String, String, u64, String, String)> {
...
pub async fn resolve_bunny(
    ...
    let (library_id, guid) = parse_bunny_embed(video_url)
        .ok_or_else(|| anyhow!("not a Bunny Stream URL: {}", video_url))?;
