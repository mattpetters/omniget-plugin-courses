...
pub fn extract_course_name(url: &str) -> Option<(String, String)> {
    let re_learn = Regex::new(r"(?i)://([^/]+?)\.udemy\.com/course/([a-zA-Z0-9_-]+)/learn").ok()?;
    ...
    let re =
        Regex::new(r"(?i)://(.+?)\.udemy\.com/(?:course(?:/draft)*/)?([a-zA-Z0-9_-]+)").ok()?;
...
async fn handle_pagination(
    ...
    let mut data: serde_json::Value = serde_json::from_str(&resp_text).map_err(|e| {
    ...
    if data.get("results").and_then(|r| r.as_array()).is_none() {
        tracing::warn!(
            "[udemy-api] response has no 'results' array. Keys: {:?}",
    ...
    while let Some(url) = next_page_url(&data) {
        if !seen.insert(url.clone()) {
            tracing::warn!(
                "[udemy-api] pagination loop detected at page {}, stopping",
                page
            );
            break;
        }
...
pub async fn list_my_courses(
    ...
    tracing::info!(
        "[udemy-api] fetching subscribed courses from {}",
        portal_name
    );
    ...
    let courses: Vec<UdemyCourse> = results.iter().filter_map(parse_course_from_json).collect();
...
pub async fn list_subscription_courses(
    ...
    tracing::info!(
        "[udemy-api] fetching subscription course enrollments from {}",
        portal_name
    );
...
pub async fn list_all_courses(
    ...
    let mut my_courses = match my_courses_result {
        ...
        Err(e) => {
            tracing::warn!(
                "[udemy-api] subscribed courses failed (continuing with subscription): {}",
                e
            );
            Vec::new()
        }
...
pub fn video_asset_is_drm_only(a: &serde_json::Value) -> bool {
    ...
    let is_drm = a
        ...
        .unwrap_or(false)
        || a.get("media_license_token")
            .map(|v| !v.is_null())
            .unwrap_or(false);

    let has_downloadable = a
        .get("media_sources")
...
pub async fn get_course_resources(
    ...
    let data = match api_get_with_retry(&session.client, &url, None).await {
        ...
        Err(e) => {
            tracing::warn!(
                "[udemy-api] resources not available for course {}: {}",
                course_id,
                e
            );
            return Ok(Vec::new());
