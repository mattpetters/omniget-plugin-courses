...
async fn fetch_courses_via_api(plugin: &crate::CoursesPlugin) -> Result<Vec<UdemyCourse>, String> {
    let session = {
        let guard = plugin.udemy_session.lock().await;
    ...
    tracing::info!(
        "[udemy-api] found {} total courses via direct API",
        courses.len()
    );
...
async fn fetch_courses(plugin: &crate::CoursesPlugin) -> Result<Vec<UdemyCourse>, String> {
    fetch_courses_via_api(plugin).await
}

pub async fn udemy_list_courses(plugin: &crate::CoursesPlugin) -> Result<Vec<UdemyCourse>, String> {
    {
