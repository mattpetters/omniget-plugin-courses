...
async fn fetch_courses_from_api(plugin: &crate::CoursesPlugin) -> Result<Vec<Course>, String> {
    ...
    let subdomains = match api::get_subdomains(session).await {
        ...
        Err(e) => {
            tracing::warn!(
                "[hotmart] check_token resources unavailable, purchases only: {}",
                e
            );
            Vec::new()
        }
    ...
    let mut courses = api::list_courses(session)
        .await
...
pub async fn hotmart_list_courses(plugin: &crate::CoursesPlugin) -> Result<Vec<Course>, String> {
    {
...
pub async fn hotmart_refresh_courses(plugin: &crate::CoursesPlugin) -> Result<Vec<Course>, String> {
    {
        let mut cache = plugin.courses_cache.lock().await;
        *cache = None;
    }
