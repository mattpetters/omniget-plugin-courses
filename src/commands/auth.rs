...
pub async fn validate_token(
    ...
    match oidc::fetch_profile(client, token).await {
        ...
        Err(e) => tracing::info!(
            "[hotmart] userinfo rejected the token ({}), trying check_token",
            e
        ),
    }
...
pub async fn hotmart_check_session(plugin: &crate::CoursesPlugin) -> Result<String, String> {
    ...
    if !has_memory_session {
        ...
        match load_saved_session().await {
            Ok(session) => {
                tracing::info!(
                    "[hotmart] check_session: loaded session for {}",
                    session.email
                );
                *plugin.hotmart_session.lock().await = Some(session);
            }
    ...
    if !session.is_expired() {
        ...
        if let Some(at) = *validated_at {
            if at.elapsed() < SESSION_COOLDOWN {
                tracing::info!(
                    "[hotmart] check_session: cooldown active, returning {}",
                    email
                );
                return Ok(email);
            }
    ...
    tracing::info!(
        "[hotmart] check_session: validating token (len={})",
        session.token.len()
    );
    match validate_token(&session.client, &session.token).await {
        ...
        Err(e) => {
            tracing::warn!(
                "[hotmart] check_session: {}; trying to renew through SSO",
                e
            );
...
pub async fn hotmart_set_cookies(
    ...
    tracing::info!(
        "[hotmart] token found: len={}, expires_at={:?}",
        token.len(),
        expires_at
    );
    ...
    let session =
        HotmartSession::from_saved(saved).map_err(|e| format!("Failed to build client: {}", e))?;

    let (kind, profile) = validate_token(&session.client, &session.token)
        .await
        .map_err(|e| {
            format!(
                "Token validation failed: {} - the session may be expired",
                e
            )
        })?;
...
mod tests {
    ...
    fn parses_netscape_export_and_key_value_lines() {
        ...
        assert_eq!(
            pairs,
            vec![("TGC".into(), "abc".into()), ("foo".into(), "bar".into())]
        );
    ...
    fn parses_object_form_and_keeps_nested_values_as_json() {
        let pairs =
            parse_cookie_input(r#"{"TGC":"abc","oidc.user:x":{"access_token":"AT"}}"#).unwrap();
    ...
    fn legacy_token_prefers_named_cookies_and_skips_storage_keys() {
        let pairs = vec![
            (
                "oidc.user:x".to_string(),
                "not-a-token-value-but-long-enough".to_string(),
            ),
